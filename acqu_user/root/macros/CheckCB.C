...
void CheckCB(){
 char *hname[] = {
   ...
   "7", "NaI_ClNhits",         "lin", "",
   "8", "NaI_ClNhitsOR",       "lin", "",
   "9", "NaI_ClPhi_v_ClTheta.G1", "log", "CONTZ",
   "10", "NaI_Hits_v_TimeOR",   "log", "COLZ",
 ...
 Char_t* xname[] = {
   ...
   "CB Distribution of Hits...Single cluster detected",
   "CB Number of Clusters per Event",
   "CB Number of Hits per Cluster",
   "CB #theta - #phi distribution of Cluster Hits...Single cluster detected",
   "CB Time vs Hits",
   "CB Energy vs Hits",
 ...
 TCanvas* canv = new TCanvas("CB-Spectra","CB-Online",240,180,1240,890);
 canv->Divide(3,4);
 i = 0;
 for( j=0; j<44; j+=4){
 ...
 for( j=44; j<60; j+=4){
   ...
   if(h2) h2->Draw(hname[j+3]);               // parameter 4th arg.
   else printf("No object named '%s' was found.\n Check your macro!\n", hname[j+1]);
   gPad->RedrawAxis();
   i++;
 }
